...
app.post('/inventario/ajuste', requireAuth, (req, res) => {
    ...
    if (!codigo) return res.status(400).json({ error: 'Código de artículo requerido' });
    ...
    if (!item) return res.status(404).json({ error: 'Artículo no encontrado en inventario' });

    const stockAnterior = parseFloat(item.stock) || 0;
    const costoAnterior = parseFloat(item.costoUnitario) || 0;
    ...
    kardex.push({
        ...
        costoUnitario: costoNuevo,
        valorTotal: Math.abs(diferencia) * costoNuevo,
        stockAnterior,
        stockNuevo,
        observaciones: motivo && motivo.trim() ? motivo.trim() : 'Ajuste manual de stock'
    });
    guardarDatos(FILES.kardex, kardex);

    // Recalcular costo en recetas que usan este insumo
    const recetas = leerDatos(FILES.recetas);
    recetas.forEach(r => {
        const detalle = JSON.parse(r.detalleReceta);
        let cambiado = false;
        const nuevoDetalle = detalle.map(ing => {
            if (ing.Codigo === codigo) {
                cambiado = true;
                return { ...ing, CostoUnitario: costoNuevo, Costo_U: costoNuevo, Subtotal: ing.Cantidad * costoNuevo };
            }
            return ing;
        });
        if (cambiado) {
...
// ==================================================
// Normaliza margenObjetivo tolerando formato viejo (decimal, ej: 0.5) y nuevo (entero, ej: 50)
function normalizarMargen(valor) {
    const v = parseFloat(valor) || 0;
    return v > 1 ? v : (v || 0.7) * 100;
}
...
function verificarCambiosCostos(receta) {
    ...
    const margenActual = receta.precioVenta > 0 
        ...
        : 0;
    const margenObjetivo = normalizarMargen(receta.margenObjetivo);
...
app.post('/recetas/crear', requireAuth, (req, res) => {
    ...
    recetas.push({
        ...
        valorUtilidad,
        margenUtilidad,
        margenObjetivo: parseFloat(margenObjetivo) > 0 ? parseFloat(margenObjetivo) : 70,
        activo: true,
        createdAt: new Date().toISOString(),
        createdBy: req.session.userId
    });
...
app.put('/recetas/:id', requireAuth, (req, res) => {
    ...
    receta.margenObjetivo = parseFloat(margenObjetivo) > 0 ? parseFloat(margenObjetivo) : normalizarMargen(receta.margenObjetivo);
    receta.updatedAt      = new Date().toISOString();
